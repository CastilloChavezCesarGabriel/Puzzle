...
class ManhattanDistanceTest(unittest.TestCase):
    ...
    def test_returns_two_when_first_two_tiles_swapped(self):
        self.assertEqual(ManhattanDistanceAlgorithm(3).estimate((2, 1, 3, 4, 5, 6, 7, 8, 0)), 2)

    def test_returns_four_when_corner_tiles_swapped_diagonally(self):
        self.assertEqual(ManhattanDistanceAlgorithm(3).estimate((5, 2, 3, 4, 1, 6, 7, 8, 0)), 4)

    def test_returns_twelve_when_empty_moves_to_start_in_3x3(self):
        self.assertEqual(ManhattanDistanceAlgorithm(3).estimate((0, 1, 2, 3, 4, 5, 6, 7, 8)), 12)

    def test_returns_sixteen_for_reversed_3x3(self):
        self.assertEqual(ManhattanDistanceAlgorithm(3).estimate((8, 7, 6, 5, 4, 3, 2, 1, 0)), 16)

    def test_returns_two_for_reversed_2x2(self):
        self.assertEqual(ManhattanDistanceAlgorithm(2).estimate((3, 2, 1, 0)), 2)

    def test_returns_six_for_fully_scrambled_2x2(self):
        self.assertEqual(ManhattanDistanceAlgorithm(2).estimate((0, 3, 2, 1)), 6)

    def test_returns_one_when_last_two_tiles_swapped_in_2x2(self):
        self.assertEqual(ManhattanDistanceAlgorithm(2).estimate((1, 2, 0, 3)), 1)

    def test_returns_one_when_last_two_tiles_swapped_in_4x4(self):
        state = tuple(range(1, 15)) + (0, 15)
        self.assertEqual(ManhattanDistanceAlgorithm(4).estimate(state), 1)

    def test_returns_one_when_last_two_tiles_swapped_in_5x5(self):
        state = tuple(range(1, 24)) + (0, 24)
        self.assertEqual(ManhattanDistanceAlgorithm(5).estimate(state), 1)

    def test_returns_two_when_two_middle_tiles_swapped_in_4x4(self):
        state = list(range(1, 16)) + [0]
        state[4], state[5] = state[5], state[4]
        self.assertEqual(ManhattanDistanceAlgorithm(4).estimate(tuple(state)), 2)

    def test_ignores_empty_tile_when_computing(self):
        self.assertEqual(ManhattanDistanceAlgorithm(3).estimate((1, 2, 3, 4, 0, 5, 6, 7, 8)), 6)
    ...
    def test_distinguishes_different_state_distances(self):
        ...
        self.assertNotEqual(ManhattanDistanceAlgorithm(3).estimate(first), ManhattanDistanceAlgorithm(3).estimate(second))
