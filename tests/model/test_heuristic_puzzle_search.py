...
class HeuristicPuzzleSearchTest(unittest.TestCase):
    def test_runs_on_already_solved_2x2(self):
        puzzle = Puzzle(2)
        self.assertIsNone(SolutionPath(HeuristicSearch(puzzle, ManhattanDistance(2)).run()).solves(puzzle))

    def test_runs_on_already_solved_3x3(self):
        puzzle = Puzzle(3)
        self.assertIsNone(SolutionPath(HeuristicSearch(puzzle, ManhattanDistance(3)).run()).solves(puzzle))

    def test_runs_on_already_solved_4x4(self):
        puzzle = Puzzle(4)
        self.assertIsNone(SolutionPath(HeuristicSearch(puzzle, ManhattanDistance(4)).run()).solves(puzzle))

    def test_runs_on_already_solved_5x5(self):
        puzzle = Puzzle(5)
        self.assertIsNone(SolutionPath(HeuristicSearch(puzzle, ManhattanDistance(5)).run()).solves(puzzle))
    ...
    def test_runs_with_zero_heuristic_on_solved_3x3(self):
        puzzle = Puzzle(3)
        self.assertIsNone(SolutionPath(HeuristicSearch(puzzle, NoHeuristic(3)).run()).solves(puzzle))
